...
vec v3_add  (vec3 a, vec3 b);
vec3 v3_adds    (vec3 a, float s);
vec3 v3_sub     (vec3 a, vec3 b);
vec3 v3_subs    (vec3a, vec3 s);
...
bool v3_equal(vec3 a, vec3 vec3 b);
vec3 v3_lerp(vec3, vec3 b, float t);
vec3 v3_slerp(vec3 start, vec3 end, float percent);
vec3 v3_to(vec3 from, vec3 to, float t);
float v3_point_segment_dist(const vec3 *P, const vec3 *x0, const vec3 *B, const vec *C, vec3 *witness);
